...
class Content extends Component {
  ...
  responseGoogle(response) {
    if (response && response.profileObj) saveUser(response.profileObj, this);
  }
    ...
    let login = (
      <div className='login_box'>
        ...
        <div className='logDiv'>
          ...
          <div
            className='checkbox'
            style={this.state.checkbox_style}
            onClick={() => {
              let new_state;
              if (!this.allow) {
                this.allow = true;
          ...
          />
          <div className='agree'>
            I agree to Smartoboto
            <br />
            <a href='https://smartoboto.co/policy' target='_blank'>
              Privacy policy
            </a>
          </div>
        </div>
        <div className='need_permission'>
          We need permissions to create your account on the system.
        </div>
      </div>
    );
    return login;
  ...
  render() {
    if (!this.props.show_regsplash) return null;
    ...
    if (success === '' || utils.getCookie('st') === 'p') {
      ...
      if (success === '') saveUser(profileObj, this);
      content = (
        <div>
          <div className='onestep'>
            Payment <span>was</span> successfull
          </div>
          <div className='ok_img' />
          <div className='description' style={{ top: 373 }}>
            Done! Now you are in our community! We will certainly notify you about opening access to
            the platform. Now you can connect your Google account for easy access. You can also
      ...
      content = (
        <div>
          ...
          <div className='description'>
            You are on the waiting list. Now we are completing the necessary work to give you access
            to the control panel as possible. Subscribe to our community
          </div>
          <div
            style={{ position: 'absolute', top: '440px', left: 'calc(50% - 60px)', width: '120px' }}
          >
            <a
              href='https://www.facebook.com/templaterobot/'
              target='_blank'
              style={{ float: 'right' }}
    ...
    let pay = (
      <div className='splashroot'>
        <a href='https://templaterobot.co' target='_blank' rel='noopener noreferrer'>
    ...
    if (this.props.show_payment) return pay;
    return login;
  }
}
...
function getQueryVariable(search_variable) {
  ...
  for (var i = 0; i < vars.length; i++) {
    ...
    if (pair[0] === search_variable) return pair[1] || '';
  }
  return null;
}
...
function saveUser(profileObj, component_obj) {
  ...
  for (let name in profileObj) FD.append(name, profileObj[name]);
