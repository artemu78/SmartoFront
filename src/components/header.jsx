...
class Header extends React.Component {
  ...
  item_template (item, i) {
    ...
    let obj = <div className={css_class} key={i} onClick={e => {
  ...
  render () {
    ...
    let header_items = [
      { name: 'My bots', action: 'SHOW_MYBOTS' },
      // {name: 'Plan', action: 'SHOW_PLAN'},
      // {name: 'Service', action: 'SHOW_SERVICE'},
      { name: 'Support', action: 'SHOW_SUPPORT' }
    ];
    ...
    let userpic = this.props.userpic ? <img alt="" className="user_pic" src={this.props.userpic} /> : null;

    return (
      <div className="header">
        <a href="/"><img alt="" src="img/logo.png" className="logo" title={pckg.version} /></a>

        <div className="items">
          {header_items2}
        </div>
        <div className={profile_class} onClick={() => {
          this.setState({ selected: 'profile' });
          this.props.dispatch({ type: 'SHOW_PROFILE' });
        }}>
          {userpic}
          <div alt="" className="user_name">{this.props.user_name}</div>
        </div>
      </div>
    );
  }
