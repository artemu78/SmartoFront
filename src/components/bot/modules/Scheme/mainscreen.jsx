...
const sortButtonsInRow = (buttons_array) => {
  ...
  buttons_array.forEach((button_obj) => {
    if (button_rows_arr[button_obj['row']] === undefined) button_rows_arr[button_obj['row']] = [];
...
const maxId = (buttons_array) => {
  let max_id = 0;
  buttons_array.forEach((button_obj) => {
    if (button_obj._id > max_id) max_id = button_obj._id;
  });
  return max_id + 1;
};
...
class MainScreen extends React.Component {
  constructor(props) {
    ...
    if (this.props.store) this.state.btnDest = this.props.store[0]._id;
  }
  ...
  saveBtn() {
    ...
    if (this.state.btnSaveMode === 'newRow') buttons.push(new_button);
      ...
      buttons = buttons.map((button_obj) =>
        button_obj._id !== new_button._id ? button_obj : new_button
      );
  ...
  delButton(id) {
    ...
    this.setState({ buttons: btn }, () => {
      this.setStore(this.props.obj._id, 'buttons', btn, this.props.store);
    });
  }
  ...
  ButtonsList() {
    ...
    const buttons_rows_html = buttons_rows_original.map((buttons_row, row_index) => {
      let buttons_elements = buttons_row.map((button_obj, button_index) => {
        ...
        return (
          <div key={randomnumber() + '.' + 1} className={scheme_style.link} title={title}>
            <div key={randomnumber() + '.' + 2} className={buttonClass}>
              {button_obj.text || '[EMPTY]'}
            </div>
            <div key={randomnumber() + '.' + 3} className={scheme_style.btn_back}>
              <span
                key={randomnumber()}
                className='add-button btn material-icons'
                title='Add new button'
                onClick={() => {
                  this.addButtonRight(row_index, button_index + 1);
                }}
              >
                add_box
              </span>
              <span
                key={randomnumber()}
                className='edit-button btn material-icons'
                title='Edit button'
                onClick={() => {
                  this.editButton(button_obj);
                }}
              >
                edit
              </span>
              <span
                key={randomnumber()}
                className='delete-button btn material-icons'
                title='Delete button'
                onClick={() => {
                  this.delButton(button_obj._id);
                }}
              >
                delete_forever
              </span>
      ...
      return (
        <li key={randomnumber()} className={scheme_style.scr_button_layout}>
          {buttons_elements}
        </li>
      );
    });
  ...
  ScreensAsSelectOptions() {
    let screens = this.props.store.map((item) => (
      <option key={randomnumber()} value={item._id}>
        {item.name}
      </option>
    ));
    screens.unshift(
      <option key={randomnumber()} value=''>
        [no destination]
      </option>
    );
    return screens;
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <div
          className={'flip-container' + (flip ? ' hover' : '')}
          style={boxSize}
          ref={(ref) => {
            this.flipContainer = ref;
          }}
        >
          <div className='flipper'>
            <div className='front' style={boxSize}>
              <div className={scheme_style.screen_control_buttons}>
                <div className={scheme_style.screen_name} title={this.state.text}>
                  {this.state.name}
                </div>
                <div>
                  <button
                  ...
                  <button
                    ...
                    title='Delete screen'
                    onClick={() => {
                      this.props.deleteScreen(this.props.obj);
                    }}
                    disabled={main}
                  >
                    clear
              ...
              <span
                className={scheme_style.wholeScreenButTitle}
                style={{ display: screenContentDisplayCSS }}
              >
                <FormControlLabel
                  control={
                    <Checkbox
                      ...
                      value='1'
                      color='default'
                      style={{ padding: '0px', paddingLeft: '11px' }}
                    />
                  }
                ...
                <textarea
                  ...
                  onChange={this.handleInput}
                  placeholder='Screen content in bot'
                  name='text'
                />
                <div className='fields_container'>
                  <input
                    className='bot-screen-input child_field'
                    value={this.state.screen_var}
                    type='text'
                    onChange={this.handleInput}
                    name='screen_var'
                ...
                <div className='fields_container'>
                  <input
                    ...
                    onChange={this.handleInput}
                    name='varname'
                    placeholder='text input in variable'
                    title='All user input gets in this variable'
                  />
                  <select
                    className='bot-screen-input child_field'
                    ...
                    onChange={this.handleInput}
                    name='module'
                    title='Show this module instead of screen content'
                  >
                    {this.modulesList()}
                  </select>
                </div>
                <div className='fields_container'>
                  <input
                    className='bot-screen-input child_field'
                    type='text'
                    value={this.state.dlgflw_intent}
                    onChange={this.handleInput}
                    name='dlgflw_intent'
                    placeholder='Dialogflow intent'
                    title='Display this screen as intent response'
                  />
                  <input
                    className='bot-screen-input child_field'
                    type='text'
                    value={this.state.next_screen}
                    onChange={this.handleInput}
                    name='next_screen'
                    placeholder='Next screen name'
                    title='Display next screen immediately'
                  />
                </div>
                <div className='buttons'>
                  <ul className='buttons_row'>{this.ButtonsList()}</ul>
                  <button
                    className={scheme_style.add_button_row}
                    onClick={() => {
                      this.addButtonNewRow();
                    }}
                  >
                    +
                  </button>
                    ...
                    <React.Fragment>
                      <div className={scheme_style.AddButton}>
                        <input
                          ...
                          className={scheme_style.btnName}
                        ...
                        />
                        <button
                          className={scheme_style.done_screen_files}
                          onClick={this.toggleDestinationSelection}
                        >
                          Destination
                        </button>
                        {this.state.showDestinationSreens ? (
                          <select
                            size='5'
                            onChange={this.handleInput}
                            value={this.state.btnDest}
                            className={scheme_style.button_data}
                            name='btnDest'
                          >
                        ...
                        <div className='fields_container'>
                          <input
                            className='bot-screen-input child_field'
                            type='text'
                            value={this.state.btnRow}
                            onChange={this.handleInput}
                            name='btnRow'
                            placeholder='Row'
                            title='0-based row position'
                          />
                          <input
                            className='bot-screen-input child_field'
                            type='text'
                            value={this.state.btnIndex}
                            onChange={this.handleInput}
                            name='btnIndex'
                            placeholder='Index'
                            title='0-based position in row'
                          />
                        </div>
                        <div>
                          <button className={scheme_style.AddButtonSave} onClick={this.saveBtn}>
                            save
                          </button>
                          <button
                            className={
                              scheme_style.AddButtonSave + ' ' + scheme_style.AddButtonCancel
                            }
                            onClick={this.cancelBtnOpts}
                          >
                            cancel
                          </button>
                        </div>
                ...
                <div className='screen-options'>
                  <div className='bot-screen-buttons-container'>
                    ...
                    >
                      <span style={{ fontSize: '17px' }} className='material-icons'>
                        attach_file
                      </span>
                      attachment ({fileListCount})
                    </button>
                  </div>
                  <button
                    title='Let your bot tell something more'
                    className='add-child-screen btn'
                    onClick={() => {
                      this.props.addScreen(this.props.obj);
                    }}
                  >
                    add child screen
                  </button>
