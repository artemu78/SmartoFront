...
class Users extends Component {
  ...
  getUserHTMLByBotType (item) {
    ...
    switch (item.bot_type) {
      case 'tg':
        ...
        userObj.link = item.username ? <a href={`https://t.me/${item.username}`} target='_blank'>{item.username}</a> : null;
        break;
      case 'vk':
        userObj.bot_type = vk_avatar;
        userObj.link = item.vk_id ? <a href={`https://vk.com/id${item.vk_id}`} target='_blank'>id{item.vk_id}</a> : null;
        break;
      case 'ws':
  ...
  componentDidMount () {
    ...
    if (window.location.port === '3000')
      this.display_users(test_users);
    else
      utils.sendRequest(obj, this.display_users, save_url);
  }
  ...
  display_users (users) {
    ...
    let usersRowsHTML = users.data.map((item, i) => {
      ...
      return <TableRow key={item.id} hover className={style.usersRow} onClick = {(e) => this.handleOpenDialog(e, item) }>
        <TableCell>{item.create_dt}</TableCell>
        <TableCell>{item.name} {item.surname} {userObj.link}</TableCell>
        <TableCell>{item.screens_count}</TableCell>
        <TableCell><img src={userObj.bot_type} alt=""/></TableCell>
      </TableRow>
    });
  ...
  displayUserDialogHTML (userDialogData) {
    if (!userDialogData || !userDialogData.data || Array.isArray(!userDialogData.data))
      return;
  ...
  getUserDialogTableHTML () {
    ...
    if (currentUser && !userDialogs.has(currentUser.id) && !userDialogRowsHTML) {
      let obj = { l: utils.getCookie('l'),
        b: this.props.bot.id,
        u: currentUser.id,
        o: 'gusd' };
      if (window.location.port === '3000')
        this.displayUserDialogHTML(test_user_dialog);
      else
        utils.sendRequest(obj, this.displayUserDialogHTML, save_url);
    }
